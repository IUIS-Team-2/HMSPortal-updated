    ...
    <div>
      ...
      <div style={{ background:"#161B2E",borderRadius:14,border:"1px solid rgba(255,255,255,.06)",overflow:"auto" }}>
        <table style={{ width:"100%",borderCollapse:"collapse",fontSize:13,minWidth:800 }}>
          <THead cols={["Patient","UHID","Diagnosis","Dept","Doctor","Admitted","Discharged","Type","Status",""]} />
          <tbody>
            {list.map(p=>(
              <tr key={p.id} style={{ borderBottom:"1px solid rgba(255,255,255,.04)" }}>
                <td style={{ padding:"12px 14px" }}><div style={{ color:"#fff",fontWeight:700 }}>{p.name}</div><div style={{ fontSize:11,color:"rgba(255,255,255,.35)" }}>{p.age}y</div></td>
                <td style={{ padding:"12px 14px",color:"rgba(255,255,255,.4)",fontSize:11 }}>{p.uhid}</td>
                <td style={{ padding:"12px 14px",color:"rgba(255,255,255,.7)",fontSize:12 }}>{p.diagnosis}</td>
                <td style={{ padding:"12px 14px" }}><Chip label={p.dept} color="#C084FC" /></td>
                <td style={{ padding:"12px 14px",color:"#38BDF8",fontSize:12 }}>{p.doctor}</td>
                <td style={{ padding:"12px 14px",color:"rgba(255,255,255,.5)",fontSize:12 }}>{p.adm}</td>
                <td style={{ padding:"12px 14px",color:"rgba(255,255,255,.5)",fontSize:12 }}>{p.dod||"—"}</td>
      ...
      {viewP&&(
        <Modal title={viewP.name} onClose={()=>setViewP(null)} wide>
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14 }}>
            {[["UHID",viewP.uhid],["Diagnosis",viewP.diagnosis],["Dept",viewP.dept],["Doctor",viewP.doctor],["Admitted",viewP.adm],["Discharged",viewP.dod||"—"],["Type",viewP.type],["Amount","₹"+viewP.amount.toLocaleString("en-IN")]].map(([k,v])=>(
              <div key={k} style={{ background:"rgba(255,255,255,.04)",borderRadius:8,padding:"9px 12px" }}>
                <div style={{ fontSize:10,color:"rgba(255,255,255,.35)",marginBottom:3 }}>{k}</div>
                <div style={{ fontSize:13,color:"#fff",fontWeight:600 }}>{v}</div>
              </div>
...
function ExpectedDOD({ patients }) {
  ...
  const diff=dod=>{if(!dod)return null;const d=new Date(dod);return Math.ceil((d-today)/(1000*60*60*24));};

  return (
    <div>
      <PageHeader title="Expected Discharge" sub="Track DOD for all admitted patients" />
      <BranchTabs active={branch} setActive={setBranch} />
      <div style={{ display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,marginBottom:18 }}>
        <Stat label="ADMITTED" value={admitted.length} icon="🛏" color="#60A5FA" />
        <Stat label="OVERDUE / TODAY" value={admitted.filter(p=>{const d=diff(p.dod);return d!==null&&d<=0;}).length} icon="🚨" color="#F87171" />
        <Stat label="WITHIN 3 DAYS" value={admitted.filter(p=>{const d=diff(p.dod);return d!==null&&d>0&&d<=3;}).length} icon="⚠️" color="#FBBF24" />
      </div>
      <div style={{ display:"flex",flexDirection:"column",gap:8 }}>
        {admitted.map(p=>{
          const d=diff(p.dod);
          const urg=d===null?"gray":d<=0?"red":d<=3?"yellow":"green";
          const col={red:{c:"#F87171",bg:"rgba(248,113,113,.12)"},yellow:{c:"#FBBF24",bg:"rgba(251,191,36,.12)"},green:{c:"#34D399",bg:"rgba(52,211,153,.12)"},gray:{c:"rgba(255,255,255,.3)",bg:"rgba(255,255,255,.04)"}}[urg];
          return (
            <div key={p.id} style={{ background:"#161B2E",borderRadius:12,padding:"14px 18px",border:`1px solid ${col.c}30`,display:"flex",alignItems:"center",gap:16 }}>
              <div style={{ flex:1 }}>
                <div style={{ fontWeight:700,color:"#fff",fontSize:14 }}>{p.name}</div>
                <div style={{ fontSize:12,color:"rgba(255,255,255,.4)",marginTop:2 }}>{p.uhid} · {p.dept} · {p.doctor}</div>
              </div>
              <div style={{ textAlign:"center" }}><div style={{ fontSize:11,color:"rgba(255,255,255,.35)",marginBottom:3 }}>ADMITTED</div><div style={{ fontSize:13,color:"rgba(255,255,255,.7)" }}>{p.adm}</div></div>
              <div style={{ textAlign:"center",background:col.bg,borderRadius:10,padding:"8px 16px",minWidth:140 }}>
                <div style={{ fontSize:11,color:"rgba(255,255,255,.35)",marginBottom:3 }}>EXP. DISCHARGE</div>
                <div style={{ fontSize:14,fontWeight:800,color:col.c }}>{p.dod||"Not Set"}</div>
                {d!==null&&<div style={{ fontSize:11,color:col.c,marginTop:2 }}>{d<0?`${Math.abs(d)}d overdue`:d===0?"Today!":d===1?"Tomorrow":`${d} days`}</div>}
              </div>
              <Chip label={p.type==="cash"?"Cash":"Cashless"} color={p.type==="cash"?"#34D399":"#60A5FA"} />
            </div>
          );
