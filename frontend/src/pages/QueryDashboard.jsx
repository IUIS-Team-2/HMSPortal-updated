    ...
    <div style={{ display:"flex", flexDirection:"column", height:"100vh", background:BG_MAIN, color:TEXT_DARK, fontFamily:"'Inter',sans-serif", overflow:"hidden" }}>
      <style>{`
        ...
        * { box-sizing:border-box; }
        ...
        .qgrid-cell { transition:background 0.1s; font-family:'JetBrains Mono',monospace; color:${TEXT_DARK}; }
        .qgrid-cell:focus { outline:2px solid ${COLOR}; outline-offset:-2px; background:#eff6ff !important; z-index:2; position:relative; }
        .qgrid-cell:hover { background:#f0f9ff !important; }

        .qrow-remove { opacity:0; transition:opacity 0.15s; }
        tr:hover .qrow-remove { opacity:1; }
        .ns_tr:hover > td { background:#f8fafc !important; }

        .qtab-btn:hover { color:${COLOR} !important; background:#f0f9ff !important; }
        .qfilter-chip:hover { border-color:${COLOR} !important; color:${COLOR} !important; background:${COLOR_PALE} !important; }
        .qaction-btn:hover { filter:brightness(0.94); transform:translateY(-1px); }
        .qaction-btn { transition:all 0.15s; }

        @keyframes qfadeIn { from{opacity:0;transform:translateY(6px)}to{opacity:1;transform:none} }
        .qfade-in { animation:qfadeIn 0.22s ease; }
      ...
      <header style={{
        ...
        <div style={{ display:"flex", alignItems:"center", gap:14 }}>
          ...
          <div>
            ...
            <div style={{ fontSize:11, color:"#93c5fd", fontWeight:500, marginTop:1 }}>IPD Portal · Query Department</div>
          </div>
        </div>
          ...
          {currentUser && (
            <div style={{ display:"flex", alignItems:"center", gap:9, paddingLeft:12, borderLeft:"1px solid rgba(255,255,255,0.15)" }}>
              ...
              <div>
                ...
                <div style={{ fontSize:10, color:"#93c5fd", fontWeight:500 }}>Query Staff</div>
              </div>
              <button onClick={onLogout} className="qaction-btn"
      ...
      <div style={{
        ...
        {viewTab === "entry" && (
          <div style={{ display:"flex", gap:8, alignItems:"center" }}>
            {hasUnsaved && <span style={{ fontSize:12, color:"#f59e0b", fontWeight:600, animation:"qpulse 2s infinite" }}>● Unsaved changes</span>}
            {savedAt && !hasUnsaved && <span style={{ fontSize:12, color:"#16a34a", fontWeight:500 }}>✓ Saved at {savedAt}</span>}
            <button className="qaction-btn" onClick={() => addRows(5)}
            ...
            <button className="qaction-btn" onClick={handleSave}
        ...
        {viewTab === "entry" && (
          <div style={{ flex:1, overflow:"auto", padding:"20px 28px" }} className="qfade-in">
            ...
            <div style={{
              ...
              <div>
                <div style={{ fontSize:14, fontWeight:700, color:TEXT_DARK }}>Today's Query Log — {today}</div>
              ...
              <div style={{ textAlign:"right" }}>
                ...
                <div style={{ fontSize:10, color:TEXT_LIGHT, marginTop:3, fontWeight:600, textTransform:"uppercase", letterSpacing:"1px" }}>Queries</div>
              </div>
            </div>
                      ...
                      return (
                        <tr key={row.id} className="ns_tr" style={{ background: filled ? "#f0f9ff" : BG_WHITE, borderBottom:`1px solid ${BORDER}` }}>
                          ...
                          <td style={{ padding:"0 6px", textAlign:"center", background: filled ? "#f0f9ff" : BG_WHITE }}>
                            <button className="qrow-remove" onClick={() => removeRow(row.id)}
                    ...
                    <tbody>
                      {filteredEntries.map((row, i) => (
                        <tr key={row.id || i} className="ns_tr" style={{ borderBottom:`1px solid ${BORDER}`, background:BG_WHITE }}>
                          ...
                          <td style={{ padding:"10px 14px", color:TEXT_MID, fontSize:11, fontFamily:"'JetBrains Mono',monospace", borderRight:`1px solid ${BORDER}` }}>{row.raiseDate || "—"}</td>
                          <td style={{ padding:"10px 14px", borderRight:`1px solid ${BORDER}` }}>
                            <span style={{ padding:"2px 10px", borderRadius:5, background:"#e0f2fe", color:"#0369a1", fontSize:11, fontFamily:"'JetBrains Mono',monospace", fontWeight:600 }}>{row.queryRepDate || "—"}</span>
                          </td>
                          <td style={{ padding:"10px 14px", color:TEXT_MID, borderRight:`1px solid ${BORDER}` }}>{row.hospital || "—"}</td>
                          <td style={{ padding:"10px 14px", color:TEXT_MID, borderRight:`1px solid ${BORDER}` }}>{row.justifyBy || "—"}</td>
                          <td style={{ padding:"10px 14px", color:TEXT_MID, borderRight:`1px solid ${BORDER}` }}>{row.replyBy || "—"}</td>
